...
To the best of my knowledge only a few multiboxers have adopted his system or the variants that I proposed,
probably because the macros and bar setup required are pretty complex. This only gets worse if you are trying
to manage more than one team, which is what led me to an automated approach.

This addon captures the same fundemental virtues, but eliminates almost all of the complexity and hassle. The
only extra manual step over a focus-based system is defining your teams with MultiboxRoster, and that's like
30 sec with a text editor.
